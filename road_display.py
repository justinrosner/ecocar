...
class Car:
    ...
    def check_out_of_screen(self):
        if self.x+self.width > 600 or self.x < 0:
            self.x -= self.dx


def check_collision(player_x, player_y, player_width, player_height, car_x,
                    car_y, car_width, car_height):
    if (player_x+player_width > car_x) and (player_x < car_x+car_width) and \
...
player = Car(175, 475, 0, 0, 110, 191, RED)
...
for i in range(car_count):
    ...
    car = Car(x, random.randrange(-150, -50), 0,
              random.randint(5, 10), 60, 60, CAR_COLOR)
    cars.append(car)
    ...
    if not collision:
        ...
        for i in range(car_count):
            if check_collision(player.x, player.y, player.width, player.height,
                               cars[i].x, cars[i].y, cars[i].width,
                               cars[i].height):
                collision = True
                pygame.mouse.set_visible(True)
                break
        ...
        pygame.draw.rect(screen, [255, 0, 0], right_signal)
